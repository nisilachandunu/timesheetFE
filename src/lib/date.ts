/**
 * Calendar helpers shared by every feature that reasons about days.
 *
 * These began life inside the timesheet feature; the time tracker groups its
 * entries by day and needs exactly the same arithmetic, and two copies of
 * timezone-sensitive date code is how one of them quietly drifts a day off.
 * `@/features/timesheet/utils` re-exports this module, so its own callers are
 * unaffected.
 */
